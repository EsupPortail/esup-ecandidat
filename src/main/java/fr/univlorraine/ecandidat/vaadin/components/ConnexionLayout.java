...
public class ConnexionLayout extends VerticalLayout {
	...
	public void addForgotCodeActivationListener(ForgotCodeActivationListener forgotCodeActivationListener){
	...
	public void updateLibelle(){
		String libBtnConnect = applicationContext.getMessage("btnConnect", null, UI.getCurrent().getLocale());
		...
		panelStudent.setCaption(applicationContext.getMessage("accueilView.title.etu", new Object[]{applicationContext.getMessage("universite.title", null, UI.getCurrent().getLocale())}, UI.getCurrent().getLocale()));
