...
public class CustomFieldGroupFieldFactory extends DefaultFieldGroupFieldFactory{
	...
	public <T extends Field> T createField(Class<?> dataType, Class<T> fieldType) {
		...
		/*Le type du champs est un ComboBoxTypeDecision-->utilise pour afficher tout les types de decsion et pas uniquement les favorables*/
			...
			return fieldType.cast(new ComboBoxTypeDecision(typeDecisionController.getTypeDecisionsEnService(),applicationContext.getMessage("validation.obigatoire", null, UI.getCurrent().getLocale())));
		}
		...
		/*La valeur est un type d'avis*/
			...
			return fieldType.cast(new RequiredComboBox<TypeAvis>(cacheController.getListeTypeAvis(),TypeAvis.class));
		}
		/*La valeur est un tag*/
		else if (dataType==Tag.class){			
			return fieldType.cast(new RequiredTagField());
		}		
		...
		/*La valeur est un type de decision*/
			...
			return fieldType.cast(new ComboBoxTypeDecision(typeDecisionController.getTypeDecisionsFavorableEnService(),applicationContext.getMessage("validation.obigatoire", null, UI.getCurrent().getLocale())));
		}
		/*La valeur est une Motivation d'Avis*/
		else if (dataType==MotivationAvis.class){
			return fieldType.cast(new ComboBoxMotivationAvis(motivationAvisController.getMotivationAvisEnService(),applicationContext.getMessage("validation.obigatoire", null, UI.getCurrent().getLocale())));
		}		
		/*La valeur est un type de diplome*/
		else if (dataType==SiScolTypDiplome.class){
			return fieldType.cast(new RequiredComboBox<SiScolTypDiplome>(cacheController.getListeTypDiplome(),SiScolTypDiplome.class));
		}
		...
		/*La valeur est un CGE*/
			...
			return fieldType.cast(new RequiredComboBox<SiScolCentreGestion>(cacheController.getListeCentreGestion(),SiScolCentreGestion.class));
		}
		/*La valeur est un type de traitement*/
		...
		/*La valeur est un type de statut de pj*/
			...
			return fieldType.cast(new RequiredComboBox<TypeStatutPiece>(tableRefController.getListeTypeStatutPieceActif(),TypeStatutPiece.class));
		}
		/*La valeur est un type de statut de dossier*/
		else if (dataType==TypeStatut.class){
			return fieldType.cast(new RequiredComboBox<TypeStatut>(cacheController.getListeTypeStatut(),TypeStatut.class));
		}
