...
public class UploadWindow extends Window {
	...
	public UploadWindow(final String prefixe, final String typeFichier, final Candidature candidature, final Boolean commune, final Boolean isOnlyImg) {
		...
		final long UPLOAD_LIMIT = parametreController.getFileMaxSize();
		...
		if (isOnlyImg) {
			...
			textLabel.setValue(applicationContext.getMessage("window.upload.message", new Object[] { UPLOAD_LIMIT }, UI.getCurrent().getLocale()));
		}
		...
		uploaderComponent.addStartedListener(e -> {
			final Integer sizeMax = fileController.getSizeMaxFileName();
			final String fileName = e.getFilename();
			if (!fileController.isFileNameOk(e.getFilename(), sizeMax)) {
				displayError(applicationContext.getMessage("window.upload.toolongfilename", new Object[] { sizeMax }, UI.getCurrent().getLocale()));
			}
			...
			/* Verif de l'extension */
				...
				if (isOnlyImg) {
					displayError(applicationContext.getMessage("window.upload.mimetype.img", null, UI.getCurrent().getLocale()));
				} else {
					displayError(applicationContext.getMessage("window.upload.mimetype", null, UI.getCurrent().getLocale()));
				}
