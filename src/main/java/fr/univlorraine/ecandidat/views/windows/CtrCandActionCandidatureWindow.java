...
public class CtrCandActionCandidatureWindow extends Window{
	 ...
	 */
	public CtrCandActionCandidatureWindow(List<Candidature> listeCandidature, List<DroitFonctionnalite> listeDroits) {		
		/* Style */
		setModal(true);
		...
		if (container.size()==0){
			...
			RequiredTagField rcbTag = (RequiredTagField) fieldGroupTag.buildAndBind(applicationContext.getMessage("candidature.action." + Candidature_.tag.getName(), null, UI.getCurrent().getLocale()), Candidature_.tag.getName());
			rcbTag.setWidthMax();			
