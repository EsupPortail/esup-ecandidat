...
public class CandidatureWindow extends Window implements CandidatureListener{
	...
	/*Champs de PJ*/
	public static final String[] FIELDS_ORDER_PJ = {PjPresentation.CHAMPS_CHECK,PjPresentation.CHAMPS_LIB_PJ,PjPresentation.CHAMPS_FILE_PJ,PjPresentation.CHAMPS_LIB_STATUT,PjPresentation.CHAMPS_CONDITIONNEL,PjPresentation.CHAMPS_COMMENTAIRE};
	public static final String[] FIELDS_ORDER_FORMULAIRE = {FormulairePresentation.CHAMPS_LIB,FormulairePresentation.CHAMPS_URL,FormulairePresentation.CHAMPS_LIB_STATUT,FormulairePresentation.CHAMPS_CONDITIONNEL,FormulairePresentation.CHAMPS_REPONSES};
	public static final String[] FIELDS_ORDER_POST_IT = {PostIt_.datCrePostIt.getName(), PostIt_.userPostIt.getName(), PostIt_.messagePostIt.getName()};
	...
	private BeanItemContainer<PostIt> postItContainer = new BeanItemContainer<PostIt>(PostIt.class);
	 ...
	 * @param listeDroit
	 */
	public CandidatureWindow(Candidature candidatureWindow, Boolean isLocked, Boolean isCanceled, Boolean archived, List<DroitFonctionnalite> listeDroit) {		
				...
				listeToCheck.forEach(pj->{
					if (pj instanceof PjPresentation) {
						if (pj.getPjCandidatFromApogee() == null){
							((PjPresentation)pj).setCheck(true);
						}else{
							((PjPresentation)pj).setCheck(false);
						}
					}
				});
		...
		new OnDemandFileDownloader(new OnDemandStreamFile() {
			@Override
			public OnDemandFile getOnDemandFile() {
				return new OnDemandFile(candidatureController.getNomFichierLettre(candidatureWindow, ConstanteUtils.TYP_LETTRE_DOWNLOAD), 
		...
		addAttachListener(e->{			
			if (isCandidatOfCandidature 
					&& !(nbPjOrFormUpdatable.getNbPjOrForm()>0)
	...
	private void updateCaptionValue() {
		...
		setCaption(caption+applicationContext.getMessage("candidature.window.title", new Object[]{candidatController.getLibelleTitle(candidature.getCandidat().getCompteMinima())}, UI.getCurrent().getLocale()));
	}
	 ...
	 */
	private HorizontalLayout getLayoutBtnConditionnel(OneClickButton btn){
	...
	private void updateBtnAction(){
		if (isAutorizedToUpdate && candidature!=null){
			...
			else if (td!=null && td.getTypeDecision().getTypeAvis().getCodTypAvis().equals(NomenclatureUtils.TYP_AVIS_FAV) && td.getTemValidTypeDecCand() 
					&& candidature.getTemAcceptCand()!=null && candidature.getTemAcceptCand() && candidature.getDatAnnulCand()==null &&					
						(
	 ...
	 */
	@Override
	public void reloadAllPiece(List<PjPresentation> listePj, Candidature candidatureLoad) {
