...
public class CtrCandPreferenceSortWindow extends Window {
	...
	public static final String[] FIELDS_ORDER = { SortOrderPresentation.CHAMPS_ORDER,
		SortOrderPresentation.CHAMPS_PROPERTY_NAME,
		SortOrderPresentation.CHAMPS_DIRECTION,
		SortOrderPresentation.CHAMPS_EXCHANGE,
		SortOrderPresentation.CHAMPS_MONTE,
		SortOrderPresentation.CHAMPS_DESCEND,
		SortOrderPresentation.CHAMPS_DELETE };
	...
	private final BeanItemContainer<SortOrderPresentation> container = new BeanItemContainer<>(SortOrderPresentation.class);
	private final Grid grid = new Grid(container);
	private final RequiredComboBox<SortOrderPresentation> rcbColonne;
	...
	public CtrCandPreferenceSortWindow(final List<Column> listeColonneView, final List<SortOrder> listeSortOrder) {
		...
		final HorizontalLayout hlAddItem = new HorizontalLayout();
		...
		final OneClickButton btnPlus = new OneClickButton(FontAwesome.PLUS_CIRCLE);
		btnPlus.addClickListener(e -> {
			if (rcbColonne.getValue() != null) {
				final SortOrderPresentation val = (SortOrderPresentation) rcbColonne.getValue();
		...
		final OneClickButton btnValid = new OneClickButton(applicationContext.getMessage("btnValid", null, UI.getCurrent().getLocale()), FontAwesome.CHECK);
	...
	private void updateComboBox() {
		final List<SortOrderPresentation> liste = new ArrayList<>(listeColonne);
	...
	private void changeRowDirection(final Item item) {
		final SortOrderPresentation bean = (SortOrderPresentation) ((BeanItem) item).getBean();
	...
	private void changeRowPosition(final Item item, final int change) {
		final Integer orderBean = ((SortOrderPresentation) ((BeanItem) item).getBean()).getOrder();
		container.getItemIds().forEach(e -> {
	...
	private void deleteRow(final Item item) {
		final SortOrderPresentation bean = (SortOrderPresentation) ((BeanItem) item).getBean();
		final Integer orderBean = bean.getOrder();
	...
	private List<SortOrderPresentation> convertSortOrderToSortOrderPresentation(List<SortOrder> listeSortOrder) {
		...
		final List<SortOrderPresentation> liste = new ArrayList<>();
		Integer i = 1;
		for (final SortOrder e : listeSortOrder) {
			final SortOrderPresentation pres = new SortOrderPresentation((String) e.getPropertyId());
	...
	private List<SortOrder> convertSortOrderPresentation(final List<SortOrderPresentation> listePres) {
		final List<SortOrder> liste = new ArrayList<>();
		listePres.forEach(e -> {
