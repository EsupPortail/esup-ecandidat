...
public class AdminCampagneWindow extends Window {
	...
	public static final String[] FIELDS_ORDER = { Campagne_.codCamp.getName(), Campagne_.libCamp.getName(),
			Campagne_.i18nLibCamp.getName(), Campagne_.datDebCamp.getName(), Campagne_.datFinCamp.getName(),
			Campagne_.datFinCandidatCamp.getName(), Campagne_.datActivatPrevCamp.getName() };
	...
	public AdminCampagneWindow(Campagne campagne, Campagne campagneAArchiver) {
		...
		for (String fieldName : fieldsOrderToUse) {
			String caption = applicationContext.getMessage("campagne.table." + fieldName, null,
		...
		if (campagneAArchiver != null) {
			LocalDateTimeField archivageField = (LocalDateTimeField) fieldGroup
			...
			layout.addComponent(new Label(applicationContext.getMessage("campagne.to.archiv",
					new Object[] { campagneAArchiver.getLibCamp() }, UI.getCurrent().getLocale())));
		}
		...
		btnEnregistrer.addClickListener(e -> {
			try {
				...
				if (!campagneController.isCodCampUnique(
						(String) fieldGroup.getField(Campagne_.codCamp.getName()).getValue(), campagne.getIdCamp())) {
