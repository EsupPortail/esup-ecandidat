...
public class AdminView extends VerticalLayout implements View, MaintenanceListener {
	...
	private OneClickButton btnMaintenance = new OneClickButton();
	...
	private TreeTable uisTreeTable = new TreeTable(null, uisContainer);
	/**
	 * Initialise la vue
	 */
	@PostConstruct
	public void init() {
		...
		if (!isLoadBalancingCandidatMode){
			...
			switchUserTextField.addShortcutListener(new ShortcutListener(null, ShortcutAction.KeyCode.ENTER, null) {
				...
				public void handleAction(Object sender, Object target) {
					String e = switchUserTextField.getValue();
		...
		OneClickButton refreshBtn = new OneClickButton(applicationContext.getMessage("admin.uiList.refresh", null, UI.getCurrent().getLocale()), FontAwesome.REFRESH);
		...
		btnKill.addClickListener(e -> {
			SessionPresentation obj = (SessionPresentation) uisTreeTable.getValue();
		...
		/*Les propriétés*/
		String idUi = MainUI.getCurrent().getUiId();
		String idSession = MainUI.getCurrent().getSession().getSession().getId();
		String idUser = userController.getCurrentUserLogin();
		...
		uisTreeTable.setCellStyleGenerator((source, itemId, propertyId) -> {
			if (propertyId!=null && propertyId.equals(ConstanteUtils.SESSION_PROPERTY_TITLE)){				
				SessionPresentation item = (SessionPresentation)itemId;				
		...
		uisTreeTable.addValueChangeListener(e -> {
			SessionPresentation selectedObject = (SessionPresentation) uisTreeTable.getValue();
			...
			boolean selectedObjectIsUserOrSessionOrUI = (
			...
			boolean selectedLock = selectedObject.getType().equals(SessionType.LOCK);
	...
	private void majContainer(){
		...
		List<SessionPresentation> listeUis = uiController.getListSessionToDisplay();
		
		/*Ajout des users*/
		listeUis.stream().filter(e->e.getType().equals(SessionType.USER)).forEach(user->{
			Item item = addItem(user);
			...
			listeUis.stream().filter(session->session.getType().equals(SessionType.SESSION) 
				...
				listeUis.stream().filter(ui->ui.getType().equals(SessionType.UI) 
					...
					/*Ajout des locks*/
					List<SessionPresentation> listeLocks = listeUis.stream().filter(lock->lock.getType().equals(SessionType.LOCK) 
	 ...
	 * @return
	 */
	private Item addItem(SessionPresentation sessionItem){
		Item item = uisContainer.addItem(sessionItem);
	...
	private void majNombreClient(){		
		...
		clientTreeTableTitle.setValue(applicationContext.getMessage("admin.uiList.title", new Object[] {((List<SessionPresentation>) uisContainer.getItemIds()).stream().filter(e->e.getType().equals(SessionType.UI)).count()}, UI.getCurrent().getLocale()));
	}
	
	/** SUpprime un elmeent de la liste
	 * @param item
	 */
	private void removeElement(SessionPresentation item){
