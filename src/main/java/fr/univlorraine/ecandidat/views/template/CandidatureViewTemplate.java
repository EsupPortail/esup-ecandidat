...
public class CandidatureViewTemplate extends VerticalLayout {
	...
	public static final String[] FIELDS_ORDER = {Candidature_.tag.getName(),
			Candidature_.candidat.getName() + "." + Candidat_.compteMinima.getName() + "."
			...
			LAST_TYPE_DECISION_PREFIXE + TypeDecisionCandidature_.typeDecision.getName() + "."
					+ TypeDecision_.libTypDec.getName(),
			LAST_TYPE_DECISION_PREFIXE + TypeDecisionCandidature_.temValidTypeDecCand.getName(),
			LAST_TYPE_DECISION_PREFIXE + TypeDecisionCandidature_.motivationAvis.getName() + "."
					+ MotivationAvis_.libMotiv.getName(),
			...
			Candidature_.temAcceptCand.getName(), Candidature_.datTransDossierCand.getName(),
			Candidature_.datReceptDossierCand.getName(), Candidature_.datCompletDossierCand.getName(),
			Candidature_.datIncompletDossierCand.getName(), Candidature_.datAnnulCand.getName(),
	 ...
	 * @param isArchived
	 */
	public void init(final Boolean modeModification, final String typGestionCandidature, final Boolean isCanceled,
		...
		/*PopUp SVA*/
		pvSva = new PopupView(applicationContext.getMessage("alertSva.popup.link", null, UI.getCurrent().getLocale()),
				null);
		...
		List<DroitFonctionnalite> listeDroitFonc = droitProfilController
				.getCandidatureFonctionnalite(typGestionCandidature, null);
		...
		if (MethodUtils.isGestionCandidatureCtrCand(typGestionCandidature)) {
			...
			securityCtrCandFonc = userController
					.getCtrCandFonctionnalite(NomenclatureUtils.FONCTIONNALITE_GEST_CANDIDATURE, auth);
			if (securityCtrCandFonc.hasNoRight()) {
				return;
			}

			/*Verification que l'utilisateur a le droit d'ouvrir la candidature*/
			SecurityCtrCandFonc openCandidatureFonc = userController
					.getCtrCandFonctionnalite(NomenclatureUtils.FONCTIONNALITE_GEST_FENETRE_CAND, auth);
			if (!openCandidatureFonc.hasNoRight()) {
				droitOpenCandidature = true;
			}

			/*Chooser de commissions*/
			List<Commission> liste = commissionController.getCommissionsEnServiceByCtrCand(
					securityCtrCandFonc.getCtrCand(), securityCtrCandFonc.getIsGestAllCommission(),
			...
			Label labelFiltre = new Label(
					applicationContext.getMessage("candidature.change.commission", null, UI.getCurrent().getLocale()));
			...
			securityCommissionFonc = userController
					.getCommissionFonctionnalite(NomenclatureUtils.FONCTIONNALITE_GEST_CANDIDATURE, auth);
			if (securityCommissionFonc.hasNoRight()) {
				return;
			}

			/*Verification que l'utilisateur a le droit d'ouvrir la candidature*/
			SecurityCommissionFonc openCandidatureFonc = userController
					.getCommissionFonctionnalite(NomenclatureUtils.FONCTIONNALITE_GEST_FENETRE_CAND, auth);
			if (!openCandidatureFonc.hasNoRight()) {
				droitOpenCandidature = true;
			}
		}
		...
		if (modeModif && listeDroitFonc.stream()
			...
			btnAction.addClickListener(e -> {
				...
				if (listeCheck.size() == 0) {
					Notification.show(
							applicationContext.getMessage("candidature.noselected", null, UI.getCurrent().getLocale()),
							Type.WARNING_MESSAGE);
				} else if (listeCheck.size() > ConstanteUtils.SIZE_MAX_EDITION_MASSE) {
					Notification.show(
							applicationContext.getMessage("candidature.toomuchselected",
									new Object[] {ConstanteUtils.SIZE_MAX_EDITION_MASSE}, UI.getCurrent().getLocale()),
							Type.WARNING_MESSAGE);
					return;
				} else {
		...
		listeCbFilter
				.add(new ComboBoxFilterPresentation(
						Candidature_.typeStatut.getName() + "." + TypeStatut_.libTypStatut.getName(),
						getComboBoxFilterComponent(
								Candidature_.typeStatut.getName() + "." + TypeStatut_.libTypStatut.getName(), null),
						null));
		listeCbFilter.add(new ComboBoxFilterPresentation(
				Candidature_.typeTraitement.getName() + "." + TypeTraitement_.libTypTrait.getName(),
				getComboBoxFilterComponent(
						Candidature_.typeTraitement.getName() + "." + TypeTraitement_.libTypTrait.getName(), null),
				null));
		listeCbFilter.add(new ComboBoxFilterPresentation(
				LAST_TYPE_DECISION_PREFIXE + TypeDecisionCandidature_.typeDecision.getName() + "."
						+ TypeDecision_.libTypDec.getName(),
		...
		btnPref.addClickListener(e -> {
			CtrCandPreferenceViewWindow window = new CtrCandPreferenceViewWindow(candidatureGrid.getColumns(),
					candidatureGrid.getFrozenColumnCount(), FIELDS_ORDER.length, candidatureGrid.getSortOrder());
			window.addPreferenceViewListener(new PreferenceViewListener() {
				...
				@Override
				public void saveInSession(final String valeurColonneVisible, final String valeurColonneOrder,
						final Integer frozenCols, final String sortColonne, final String sortDirection) {
					preferenceController.savePrefCandInSession(valeurColonneVisible, valeurColonneOrder, frozenCols,
							sortColonne, sortDirection, true);
					candidatureGrid.setFrozenColumnCount(frozenCols);
				}

				@Override
				public void saveInDb(final String valeurColonneVisible, final String valeurColonneOrder,
						final Integer frozenCols, final String sortColonne, final String sortDirection) {
					preferenceController.savePrefCandInDb(valeurColonneVisible, valeurColonneOrder, frozenCols,
							sortColonne, sortDirection);
					candidatureGrid.setFrozenColumnCount(frozenCols);
				}
				...
				public void initPref() {
					...
					candidatureGrid.initColumn(FIELDS_ORDER, fieldsOrderVisibletoUse, orderColonne,
							"candidature.table.", Candidature_.idCand.getName(), sortDirection, listeCbFilter);
					/*candidatureGrid.initColumnVisible(FIELDS_ORDER, fieldsOrderVisibletoUse);
					candidatureGrid.initColumnOrder(FIELDS_ORDER);
					candidatureGrid.initSort(Candidature_.idCand.getName(), null, null);*/
		...
		if (nb.equals(1)) {
			btnDownload.setDescription(
					applicationContext.getMessage("candidature.download.btn", null, UI.getCurrent().getLocale()));
		} else {
			btnDownload.setDescription(applicationContext.getMessage("candidature.download.multiple.btn",
					new Object[] {nb}, UI.getCurrent().getLocale()));
		}

		new OnDemandFileDownloader(new OnDemandStreamFile() {
			@Override
			public OnDemandFile getOnDemandFile() {
				OnDemandFile file = candidatureController.downlaodMultipleDossier(getListeCandidatureSelected(),
						getCommission());
				if (file != null) {
		...
		candidatureGrid.initColumn(FIELDS_ORDER, visibleColonne, orderColonne, "candidature.table.", sortColonne,
				sortDirection, listeCbFilter);
		...
		if (modeModif) {
			...
			candidatureGrid.addSelectionListener(e -> {
				if (candidatureGrid.getSelectedRows().size() == ConstanteUtils.SIZE_MAX_EDITION_MASSE) {
					Notification.show(
							applicationContext.getMessage("candidature.maxselected",
									new Object[] {ConstanteUtils.SIZE_MAX_EDITION_MASSE}, UI.getCurrent().getLocale()),
		...
		candidatureGrid.addItemClickListener(e -> {
			...
			if (modeModif) {
				...
				try {
					...
					Notification.show(applicationContext.getMessage("candidature.select.error", null,
				...
				if (droitOpenCandidatureFinal && e.isDoubleClick()) {
					if (e.getItemId() instanceof Candidature) {
						candidatureController.openCandidatureGestionnaire((Candidature) e.getItemId(), isCanceled,
								isArchived, listeDroitFonc);
					}
				}
				majNbCandidatures();
			} else {
		...
		candidatureGrid.setColumnWidth(Candidature_.candidat.getName() + "." + Candidat_.compteMinima.getName() + "."
				+ CompteMinima_.numDossierOpiCptMin.getName(), 120);
		...
		candidatureGrid.setColumnWidth(
				LAST_TYPE_DECISION_PREFIXE + TypeDecisionCandidature_.commentTypeDecCand.getName(), 120);
		candidatureGrid.setColumnWidth(LAST_TYPE_DECISION_PREFIXE + TypeDecisionCandidature_.motivationAvis.getName()
				+ "." + MotivationAvis_.libMotiv.getName(), 120);
		candidatureGrid.setColumnWidth(Candidature_.typeStatut.getName() + "." + TypeStatut_.libTypStatut.getName(),
				145);
		candidatureGrid.setColumnWidth(
				Candidature_.typeTraitement.getName() + "." + TypeTraitement_.libTypTrait.getName(), 157);
	...
	private void setButtonState(final Boolean droitOpenCandidature, final List<DroitFonctionnalite> listeDroitFonc) {
		...
		if (nbCandidaturesSelected == 1) {
			btnAction.setEnabled(true);
				...
				.filter(e -> !e.getCodFonc().equals(NomenclatureUtils.FONCTIONNALITE_VISU_HISTO_AVIS)
	...
	private Candidature getCandidatureSelected() {
		...
		if (liste.size() == 1) {
			...
			Candidature candi = ((BeanItem<Candidature>) candidatureGrid.getContainerDataSource().getItem(liste.get(0)))
					.getBean();
			return candi;
	...
	private void majNbCandidatures() {
		nbCandidatureLabel.setValue(applicationContext.getMessage("candidature.table.nombre", new Object[] {
				candidatureGrid.getContainerDataSource().getItemIds().size(), getListeCandidatureSelected().size()},
	...
	protected void setTitle(final String code) {
		if ((securityCtrCandFonc == null || securityCtrCandFonc.hasNoRight())
				&& (securityCommissionFonc == null || securityCommissionFonc.hasNoRight())) {
			return;
		} else if (securityCtrCandFonc != null && !securityCtrCandFonc.hasNoRight()) {
			titleView.setValue(applicationContext.getMessage(code,
			...
			titleView.setValue(applicationContext.getMessage("candidature.commission.title",
					new Object[] {securityCommissionFonc.getCommission().getLibComm()}, UI.getCurrent().getLocale()));
		}
	 ...
	 */
	public String getStyleSva(final Candidature candidature, final List<AlertSva> listeAlerteSva, final String dateSva,
			final Boolean definitifSva) {
		...
		if (definitifSva) {
			if (typeDecisionCandidature != null && typeDecisionCandidature.getTemValidTypeDecCand()
					&& typeDecisionCandidature.getTypeDecision().getTemDefinitifTypDec()) {
				return null;
			}
		}
		// autres cas
