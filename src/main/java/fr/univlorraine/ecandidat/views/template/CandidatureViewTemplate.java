...
public class CandidatureViewTemplate extends VerticalLayout{
	...
	public static final String[] FIELDS_ORDER_VISIBLE = {
			Candidature_.tag.getName(),
			Candidature_.candidat.getName()+"."+Candidat_.compteMinima.getName()+"."+CompteMinima_.numDossierOpiCptMin.getName(),
			Candidature_.candidat.getName()+"."+Candidat_.nomPatCandidat.getName(),
			Candidature_.candidat.getName()+"."+Candidat_.prenomCandidat.getName(),
	...
	private GridFormatting<Candidature> candidatureGrid = new GridFormatting<Candidature>(Candidature.class);
	 ...
	 * @param isArchived
	 */
	public void init(Boolean modeModification, String typGestionCandidature, Boolean isCanceled, Boolean isArchived) {
		...
		if (MethodUtils.isGestionCandidatureCtrCand(typGestionCandidature)){			
			...
			List<Commission> liste = commissionController.getCommissionsEnServiceByCtrCand(securityCtrCandFonc.getCtrCand(),securityCtrCandFonc.getIsGestAllCommission(),securityCtrCandFonc.getListeIdCommission());
		...
		List<ComboBoxFilterPresentation> listeCbFilter = new ArrayList<ComboBoxFilterPresentation>();
		
		listeCbFilter.add(new ComboBoxFilterPresentation(Candidature_.typeStatut.getName()+"."+TypeStatut_.libTypStatut.getName(),getComboBoxFilterComponent(Candidature_.typeStatut.getName()+"."+TypeStatut_.libTypStatut.getName(), null), null));
		listeCbFilter.add(new ComboBoxFilterPresentation(Candidature_.typeTraitement.getName()+"."+TypeTraitement_.libTypTrait.getName(),getComboBoxFilterComponent(Candidature_.typeTraitement.getName()+"."+TypeTraitement_.libTypTrait.getName(), null), null));		
		listeCbFilter.add(new ComboBoxFilterPresentation(LAST_TYPE_DECISION_PREFIXE+TypeDecisionCandidature_.typeDecision.getName()+"."+TypeDecision_.libTypDec.getName(), getComboBoxFilterComponent(LAST_TYPE_DECISION_PREFIXE+TypeDecisionCandidature_.typeDecision.getName()+"."+TypeDecision_.libTypDec.getName(), libFilterNull), libFilterNull));
		
		
		/* La colonne de tag n'est plus automatiquement visibles si aucun tags en service*/
		final String[] fieldsOrderVisibletoUse = (cacheController.getTagEnService().size() != 0)?FIELDS_ORDER_VISIBLE:(String[])ArrayUtils.removeElement(FIELDS_ORDER_VISIBLE, Candidature_.tag.getName());
		...
		new OnDemandFileDownloader(new OnDemandStreamFile() {
			...
			public OnDemandFile getOnDemandFile() {
				...
				OnDemandFile file = candidatureController.downloadDossier(candidature, candidatureController.getInformationsCandidature(candidature, false), candidatureController.getInformationsDateCandidature(candidature, false), 
		...
		/*candidatureGrid.initColumn(container, FIELDS_ORDER, visibleColonne, orderColonne,"candidature.table.");
		candidatureGrid.initSort(Candidature_.idCand.getName(), sortColonne, sortDirection);*/
		
		/*On formate les types*/
		/*candidatureGrid.addBooleanColumns(Candidature_.temValidTypTraitCand.getName(), LAST_TYPE_DECISION_PREFIXE+TypeDecisionCandidature_.temValidTypeDecCand.getName(), Candidature_.candidat.getName()+"."+Candidat_.compteMinima.getName()+"."+CompteMinima_.temFcCptMin.getName(),Candidature_.temAcceptCand.getName());		
		candidatureGrid.addLocalDateFormatingColumns(Candidature_.datReceptDossierCand.getName(), 
				Candidature_.datCompletDossierCand.getName(), 
				Candidature_.datIncompletDossierCand.getName(),
				LAST_TYPE_DECISION_PREFIXE+TypeDecisionCandidature_.preselectDateTypeDecCand.getName());
		candidatureGrid.addLocalDateTimeFormatingColumns(Candidature_.datTransDossierCand.getName());*/
		...
		/*Les filtres sur la grid*/
		//HeaderRow filterRow = candidatureGrid.appendHeaderRow();
		
		/*Filtres de String*/
		/*candidatureGrid.addStringFilters(filterRow, container, 
				Candidature_.candidat.getName()+"."+Candidat_.compteMinima.getName()+"."+CompteMinima_.numDossierOpiCptMin.getName(),
				Candidature_.candidat.getName()+"."+Candidat_.nomPatCandidat.getName(),
				Candidature_.candidat.getName()+"."+Candidat_.prenomCandidat.getName(),
				Candidature_.formation.getName()+"."+Formation_.codForm.getName(),
		...
		/*Filtres de Date*/
		/*candidatureGrid.addDateFilter(filterRow, container,
				Candidature_.datTransDossierCand.getName(),
				Candidature_.datReceptDossierCand.getName(),
				Candidature_.datCompletDossierCand.getName(),
				Candidature_.datIncompletDossierCand.getName()
		...
		/*Filtres de Boolean*/
		/*candidatureGrid.addBooleanFilters(filterRow, container,
				Candidature_.temValidTypTraitCand.getName(),
				Candidature_.candidat.getName()+"."+Candidat_.compteMinima.getName()+"."+CompteMinima_.temFcCptMin.getName(),
				LAST_TYPE_DECISION_PREFIXE+TypeDecisionCandidature_.temValidTypeDecCand.getName());*/
		
		/*Filtre spécial pour la confirmation-->on teste aussi les nulls*/
		/*candidatureGrid.addBooleanFilter(filterRow, container, Candidature_.temAcceptCand.getName(), 
				applicationContext.getMessage("confirm.label", null, UI.getCurrent().getLocale()),
				applicationContext.getMessage("desist.label", null, UI.getCurrent().getLocale()),
				applicationContext.getMessage("attente.label", null, UI.getCurrent().getLocale()));*/
	 ...
	 */
	private void setButtonState(Boolean droitOpenCandidature, List<DroitFonctionnalite> listeDroitFonc){
		...
		if (nbCandidaturesSelected==1){
			btnAction.setEnabled(true);
		}else if (candidatureGrid.getSelectedRows().size()>1 && (listeDroitFonc.stream().filter(e->
				...
				&& !e.getCodFonc().equals(NomenclatureUtils.FONCTIONNALITE_OPEN_CANDIDAT)).count()>0)){
				btnAction.setEnabled(true);
		}	else{			
			btnAction.setEnabled(false);
		}
	...
	private Candidature getCandidatureSelected(){
		...
		if (liste.size()==1){
			...
			Candidature candi = (Candidature) ((BeanItem<Candidature>)candidatureGrid.getContainerDataSource().getItem(liste.get(0))).getBean();
			return candi;
	...
	private void majNbCandidatures(){
		nbCandidatureLabel.setValue(applicationContext.getMessage("candidature.table.nombre", new Object[]{
				candidatureGrid.getContainerDataSource().getItemIds().size(),
				getListeCandidatureSelected().size()
	 ...
	 * @param code
	 */
	protected void setTitle(String code){		
	 ...
	 * Ajoute les alertes SVA
	 */
	protected void addAlertSva(Boolean isCanceled, Boolean isArchived){
	 ...
	 */
	private Content createPopUpContent(List<AlertSva> listeAlerteSva, String dateSva){
		...
		listeAlerteSva.forEach(alert->{
			vlAlert.addComponent(new Label("<div style='display:inline-block;border:1px solid;width:20px;height:20px;background:"+alert.getColorSva()+";'></div><div style='height:100%;display: inline-block;vertical-align: super;'>"+applicationContext.getMessage("alertSva.popup.alert", new Object[]{alert.getNbJourSva()}, UI.getCurrent().getLocale())+"</div>", ContentMode.HTML));
		});
		return new Content() {
            /**serialVersionUID**/
			private static final long serialVersionUID = -4599757106887300854L;
	 ...
	 */
	public ComboBox getComboBoxFilterComponent(Object propertyId, String libNull) {
	 ...
	 */
	private ComboBox generateComboBox(List<String> liste, String libNull) {
	 ...
	 */
	public String getStyleSva(Candidature candidature, List<AlertSva> listeAlerteSva, String dateSva, Boolean definitifSva) {
