...
public interface CentreCandidatureRepository extends JpaRepository<CentreCandidature, Integer> {
	...
	/* Onglet Stat */
	@Query("select ce.idCtrCand, count(1) from Candidature ca"
			+ " join ca.formation fo"
			+ " join fo.commission co"
			+ " join co.centreCandidature ce"
			+ " join ca.candidat cand"
			+ " join cand.compteMinima cpt"
			+ " where cpt.campagne.idCamp = :idcamp"
			+ " group by ce.idCtrCand")
	List<Object[]> findStatNbCandidature(@Param("idcamp") Integer idcamp);
	
	@Query("select ce.idCtrCand, ca.typeStatut.codTypStatut, count(1) from Candidature ca"
			+ " join ca.formation fo"
			+ " join fo.commission co"
			+ " join co.centreCandidature ce"
			+ " join ca.candidat cand"
	...
	@Query("select ce.idCtrCand, ca.temAcceptCand, count(1) from Candidature ca"
			+ " join ca.formation fo"
			+ " join fo.commission co"
			+ " join co.centreCandidature ce"
			+ " join ca.candidat cand"
			...
			+" group by ce.idCtrCand, ca.temAcceptCand")
	List<Object[]> findStatNbCandidatureByConfirm(@Param("idcamp") Integer idcamp);
	
	@Query("select ce.idCtrCand, ty.codTypAvis, td.temValidTypeDecCand, count(1)"
