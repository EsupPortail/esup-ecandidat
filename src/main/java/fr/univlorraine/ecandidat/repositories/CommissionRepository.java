...
public interface CommissionRepository extends JpaRepository<Commission, Integer> {
	...
	List<Commission> findByCentreCandidatureIdCtrCandAndTesCommAndCentreCandidatureTesCtrCand(Integer idCtrCand,
			Boolean tesComm, Boolean tesCtrCand);
	...
	List<Commission> findByCentreCandidatureIdCtrCandAndIdCommIn(@Param("idCtrCand") Integer idCtrCand,
			@Param("listeIdCommission") List<Integer> listeIdCommission);
	...
	List<Commission> findByCentreCandidatureIdCtrCandAndTesCommAndIdCommIn(@Param("idCtrCand") Integer idCtrCand,
			@Param("tesComm") Boolean tesComm, @Param("listeIdCommission") List<Integer> listeIdCommission);
	...
	@Query("select co.idComm, ca.typeStatut.codTypStatut, count(1) from Candidature ca" + " join ca.formation fo"
			...
			+ " and ca.datAnnulCand is null" + " group by co.idComm, ca.typeStatut.codTypStatut")
	List<Object[]> findStatNbCandidatureByStatut(@Param("idCtrCand") Integer idCtrCand,
			@Param("idcamp") Integer idcamp);

	@Query("select co.idComm, ca.temAcceptCand, count(1) from Candidature ca" + " join ca.formation fo"
			+ " join fo.commission co" + " join co.centreCandidature ce" + " join ca.candidat cand"
			+ " join cand.compteMinima cpt" + " where ce.idCtrCand = :idCtrCand and cpt.campagne.idCamp = :idcamp"
			+ " and ca.temAcceptCand is not null" + " and ca.datAnnulCand is null"
			+ " group by co.idComm, ca.temAcceptCand")
	List<Object[]> findStatNbCandidatureByConfirm(@Param("idCtrCand") Integer idCtrCand,
			@Param("idcamp") Integer idcamp);

	@Query("select co.idComm, ty.codTypAvis, td.temValidTypeDecCand, count(1)" + " from TypeDecisionCandidature td"
