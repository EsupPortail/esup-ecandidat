...
public class CandidatController {
	...
	public Object getLibelleTitle(CompteMinima cptMin) {
		if (cptMin.getCandidat() == null) {
			...
			return candidat.getNomPatCandidat() + " " + candidat.getPrenomCandidat() + " ("
	 ...
	 */
	private Boolean isRessourceLocked(CompteMinima cptMin, String ressource) {
	...
	private String getLienValidation(String numDossier) {
	...
	public CompteMinima searchCptMinByEMail(String eMail) {
		...
		return compteMinimaRepository.findByMailPersoCptMinIgnoreCaseAndCampagneCodCamp(eMail,
	...
	public CompteMinima searchCptMinByLogin(String username) {
		...
		return compteMinimaRepository.findByLoginCptMinIgnoreCaseAndCampagneCodCamp(username,
				campagneEnCours.getCodCamp());
	}
	...
	public CompteMinima searchCptMinByNumDossier(String numDossier) {
		...
		return compteMinimaRepository.findByNumDossierOpiCptMinAndCampagneCodCamp(numDossier,
				campagneEnCours.getCodCamp());
	}
	 ...
	 */
	public Boolean initPasswordOrActivationCode(String eMail, String mode) {
		// Generateur de mot de passe
		PasswordHashService passwordHashUtils = PasswordHashService.getCurrentImplementation();
		...
		if (cptMin == null) {
			Notification.show(
					applicationContext.getMessage("compteMinima.id.oublie.mail.err", null, UI.getCurrent().getLocale()),
					Type.WARNING_MESSAGE);
			return false;
		}
		String pwd = passwordHashUtils.generateRandomPassword(ConstanteUtils.GEN_SIZE, ConstanteUtils.GEN_PWD);
		...
		if (mode.equals(ConstanteUtils.FORGOT_MODE_ID_OUBLIE)) {
			CptMinMailBean mailBean = new CptMinMailBean(cptMin.getPrenomCptMin(), cptMin.getNomCptMin(),
					cptMin.getNumDossierOpiCptMin(), pwd, null,
					campagneController.getLibelleCampagne(cptMin.getCampagne(), getCodLangueCptMin(cptMin)), null);
			mailController.sendMailByCod(cptMin.getMailPersoCptMin(), NomenclatureUtils.MAIL_CPT_MIN_ID_OUBLIE,
					mailBean, null, getCodLangueCptMin(cptMin));
			Notification.show(
					applicationContext.getMessage("compteMinima.id.oublie.success", null, UI.getCurrent().getLocale()),
					Type.HUMANIZED_MESSAGE);
		} else {
			CptMinMailBean mailBean = new CptMinMailBean(cptMin.getPrenomCptMin(), cptMin.getNomCptMin(),
					cptMin.getNumDossierOpiCptMin(), pwd, getLienValidation(cptMin.getNumDossierOpiCptMin()),
					campagneController.getLibelleCampagne(cptMin.getCampagne(), getCodLangueCptMin(cptMin)),
					formatterDate.format(cptMin.getDatFinValidCptMin()));
			mailController.sendMailByCod(cptMin.getMailPersoCptMin(), NomenclatureUtils.MAIL_CPT_MIN, mailBean, null,
					getCodLangueCptMin(cptMin));
			Notification.show(applicationContext.getMessage("compteMinima.code.oublie.success", null,
					UI.getCurrent().getLocale()), Type.HUMANIZED_MESSAGE);
		}
	...
	public List<SimpleTablePresentation> getInformationsPerso(Candidat candidat) {
		...
		liste.add(new SimpleTablePresentation(1, Candidat_.siScolPaysNat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.siScolPaysNat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getSiScolPaysNat().getLibNat()));
		liste.add(new SimpleTablePresentation(2, Candidat_.civilite.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.civilite.getName(), null, UI.getCurrent().getLocale()),
				candidat.getCivilite().getCodCiv()));
		liste.add(new SimpleTablePresentation(3, Candidat_.nomPatCandidat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.nomPatCandidat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getNomPatCandidat()));
		liste.add(new SimpleTablePresentation(4, Candidat_.nomUsuCandidat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.nomUsuCandidat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getNomUsuCandidat()));
		liste.add(new SimpleTablePresentation(5, Candidat_.prenomCandidat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.prenomCandidat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getPrenomCandidat()));
		liste.add(new SimpleTablePresentation(6, Candidat_.autrePrenCandidat.getName(),
				applicationContext.getMessage("infoperso.table." + Candidat_.autrePrenCandidat.getName(), null,
						UI.getCurrent().getLocale()),
				candidat.getAutrePrenCandidat()));
		liste.add(new SimpleTablePresentation(7, Candidat_.ineCandidat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.ineCandidat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getIneCandidat()));
		liste.add(new SimpleTablePresentation(7, Candidat_.cleIneCandidat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.cleIneCandidat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getCleIneCandidat()));
		liste.add(new SimpleTablePresentation(8, Candidat_.telCandidat.getName(), applicationContext
				.getMessage("infoperso.table." + Candidat_.telCandidat.getName(), null, UI.getCurrent().getLocale()),
				candidat.getTelCandidat()));
		liste.add(new SimpleTablePresentation(9, Candidat_.telPortCandidat.getName(),
				applicationContext.getMessage("infoperso.table." + Candidat_.telPortCandidat.getName(), null,
						UI.getCurrent().getLocale()),
				candidat.getTelPortCandidat()));
		liste.add(new SimpleTablePresentation(10, Candidat_.datNaissCandidat.getName(),
				applicationContext.getMessage("infoperso.table." + Candidat_.datNaissCandidat.getName(), null,
						UI.getCurrent().getLocale()),
		...
		liste.add(new SimpleTablePresentation(11, Candidat_.siScolPaysNaiss.getName(),
				applicationContext.getMessage("infoperso.table." + Candidat_.siScolPaysNaiss.getName(), null,
						UI.getCurrent().getLocale()),
				candidat.getSiScolPaysNaiss().getLibPay()));
		liste.add(new SimpleTablePresentation(12, Candidat_.siScolDepartement.getName(),
				applicationContext.getMessage("infoperso.table." + Candidat_.siScolDepartement.getName(), null,
						UI.getCurrent().getLocale()),
		...
		liste.add(new SimpleTablePresentation(13, Candidat_.libVilleNaissCandidat.getName(),
				applicationContext.getMessage("infoperso.table." + Candidat_.libVilleNaissCandidat.getName(), null,
						UI.getCurrent().getLocale()),
				candidat.getLibVilleNaissCandidat()));
		if (cacheController.getLangueEnServiceWithoutDefault().size() > 0) {
			liste.add(new SimpleTablePresentation(14, Candidat_.langue.getName(), applicationContext
					.getMessage("infoperso.table." + Candidat_.langue.getName(), null, UI.getCurrent().getLocale()),
					candidat.getLangue()));
		}
		return liste;
	}
	...
	public void nettoyageCptMinInvalides() {
		List<CompteMinima> listeToDelete = compteMinimaRepository.findByTemValidCptMinAndDatFinValidCptMinBefore(false,
				LocalDateTime.now());
	 ...
	 */
	@Transactional
	private void nettoyageCptMinInvalide(CompteMinima cptMin) {
		if (cptMin.getCandidat() == null) {
	 ...
	 * @param listener
	 */
	private Candidat saveCandidat(Candidat candidat, WSIndividu individu, CandidatAdminListener listener) {
		candidat = candidatRepository.save(candidat);
		CompteMinima cptMin = candidat.getCompteMinima();
		if (individu != null && individu.getCodEtu() != null
				&& !String.valueOf(individu.getCodEtu()).equals(cptMin.getSupannEtuIdCptMin())) {
	...
	public void editCandidat(CompteMinima cptMin, InfoPersoListener listener) {
		...
		window.addCandidatWindowListener((cand, individuApogee, needToDeleteDataApogee) -> {
			...
			cand.setIneCandidat(MethodUtils.cleanForApogee(cand.getIneCandidat()));
			cand.setCleIneCandidat(MethodUtils.cleanForApogee(cand.getCleIneCandidat()));
			...
			candidatSave.setCandidatBacOuEqu(candidatParcoursController.getBacByApogeeData(
					(individuApogee != null) ? individuApogee.getBac() : null, candidatSave, needToDeleteDataApogee));
			if (parametreController.getIsGetCursusInterne()) {
				candidatSave.setCandidatCursusInternes(candidatParcoursController.getCursusInterne(
						(individuApogee != null) ? individuApogee.getListCursusInterne() : null, candidatSave,
						needToDeleteDataApogee));
			}
	...
	private Adresse getAdresseByApogeeData(WSAdresse adr) {
		...
		return new Adresse(adr.getLibAd1(), adr.getLibAd2(), adr.getLibAd3(), adr.getCodBdi(), adr.getLibAde(), commune,
				pays);
	}
	...
	public void editMail(CompteMinima cptMin) {
		...
		cptMinWin.addCompteMinimaWindowListener(e -> {
			...
			compteMinimaRepository.save(cptMin);
			String base64encodedString = Base64.getUrlEncoder().withoutPadding()
					.encodeToString(cptMin.getNumDossierOpiCptMin().getBytes());
			String path = loadBalancingController.getApplicationPathForCandidat() + "rest/candidat/mail/"
					+ base64encodedString;
			CptMinMailBean mailBean = new CptMinMailBean(cptMin.getPrenomCptMin(), cptMin.getNomCptMin(),
					cptMin.getNumDossierOpiCptMin(), null, path,
					campagneController.getLibelleCampagne(cptMin.getCampagne(), getCodLangueCptMin(cptMin)), null);

			mailController.sendMailByCod(cptMin.getMailPersoCptMin(), NomenclatureUtils.MAIL_CPT_MIN_MOD_MAIL, mailBean,
					null, getCodLangueCptMin(cptMin));
			Notification.show(
					applicationContext.getMessage("compteMinima.editmail.notif", null, UI.getCurrent().getLocale()),
					Type.WARNING_MESSAGE);
			MainUI.getCurrent().reconstructMainMenu();
		});
		UI.getCurrent().addWindow(cptMinWin);
	 ...
	 */
	private String getCodLangueCptMin(CompteMinima cptMin) {
		if (cptMin.getCandidat() != null) {
	...
	public Boolean getINEObligatoire(SiScolPays nationalite) {
		if (parametreController.getIsIneObligatoireFr() && nationalite != null
				&& nationalite.getCodPay().equals(ConstanteUtils.PAYS_CODE_FRANCE)) {
			return true;
		}
		return false;
	}
	...
	public Boolean checkDateNaissance(LocalDate dateNaissance) {
		...
		if (dateNaissance.isBefore(dateDeb) || dateNaissance.isAfter(dateFin)) {
			Notification.show(
					applicationContext.getMessage("infoperso.datnaiss.invalid", null, UI.getCurrent().getLocale()),
					Type.WARNING_MESSAGE);
			return false;
		}
		return true;
	 ...
	 * @param exactSearch
	 * @return la liste des compte grace a un filtre pour la recherche de compte
	 *         candidat
	 */
	public List<CompteMinima> getCptMinByFilter(String value, Boolean inclureDossierOtherYears, Boolean exactSearch) {
		if (!exactSearch) {
	...
	public List<SimpleTablePresentation> getInfoForAdmin(CompteMinima cptMin) {
		...
		liste.add(new SimpleTablePresentation(1, CompteMinima_.nomCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.nomCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getNomCptMin()));
		liste.add(new SimpleTablePresentation(2, CompteMinima_.prenomCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.prenomCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getPrenomCptMin()));
		liste.add(new SimpleTablePresentation(3, CompteMinima_.mailPersoCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.mailPersoCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getMailPersoCptMin()));
		liste.add(new SimpleTablePresentation(4, CompteMinima_.numDossierOpiCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.numDossierOpiCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getNumDossierOpiCptMin()));
		liste.add(new SimpleTablePresentation(5, CompteMinima_.loginCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.loginCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getLoginCptMin()));
		liste.add(new SimpleTablePresentation(6, CompteMinima_.supannEtuIdCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.supannEtuIdCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getSupannEtuIdCptMin()));
		liste.add(new SimpleTablePresentation(7, CompteMinima_.datCreCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.datCreCptMin.getName(), null,
						UI.getCurrent().getLocale()),
		...
		liste.add(new SimpleTablePresentation(8, CompteMinima_.datFinValidCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.datFinValidCptMin.getName(), null,
						UI.getCurrent().getLocale()),
		...
		liste.add(new SimpleTablePresentation(9, CompteMinima_.temValidCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.temValidCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getTemValidCptMin()));
		liste.add(new SimpleTablePresentation(10, CompteMinima_.temValidMailCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.temValidMailCptMin.getName(), null,
						UI.getCurrent().getLocale()),
				cptMin.getTemValidMailCptMin()));
		liste.add(new SimpleTablePresentation(10, CompteMinima_.temFcCptMin.getName(),
				applicationContext.getMessage("compteMinima.table." + CompteMinima_.temFcCptMin.getName(), null,
						UI.getCurrent().getLocale()),
	...
	public void synchronizeCandidat(CompteMinima cptMin, CandidatAdminListener listener) {
		...
		ConfirmWindow win = new ConfirmWindow(
				applicationContext.getMessage("candidat.sync.apo.window", null, UI.getCurrent().getLocale()));
		win.addBtnOuiListener(e -> {
			...
			try {
				...
				if (cptMin != null && cptMin.getSupannEtuIdCptMin() != null
						&& !cptMin.getSupannEtuIdCptMin().equals("")) {
					individu = recupInfoCandidat(cptMin.getSupannEtuIdCptMin(), null, null);
					if (individu != null
							&& isINEPresent(individu.getCodNneInd(), individu.getCodCleNneInd(), candidat)) {
						Notification.show(applicationContext.getMessage("infoperso.ine.allready.present", null,
								UI.getCurrent().getLocale()), Type.WARNING_MESSAGE);
						return;
					}
				} else if (candidat != null && candidat.getIneCandidat() != null
						&& !candidat.getIneCandidat().equals("") && candidat.getCleIneCandidat() != null
					...
					if (ine == null || ine.equals("") || cleIne == null || cleIne.equals("")) {
						Notification.show(applicationContext.getMessage("candidat.sync.apo.ine.absent", null,
								UI.getCurrent().getLocale()), Type.WARNING_MESSAGE);
						return;
					}
				...
				if (individu != null) {
					...
					if (MethodUtils.validateBean(candidat, logger)) {
						...
						if (parametreController.getIsGetCursusInterne()) {
							candidatParcoursController.getCursusInterne(individu.getListCursusInterne(), candidat,
									true);
						}
					} else {
						Notification.show(applicationContext.getMessage("candidat.sync.apo.etu.nok", null,
								UI.getCurrent().getLocale()), Type.WARNING_MESSAGE);
						return;
					}
				} else {
					Notification.show(applicationContext.getMessage("candidat.sync.apo.etu.absent", null,
							UI.getCurrent().getLocale()), Type.WARNING_MESSAGE);
					return;
				}

			} catch (SiScolException f) {
				logger.error(applicationContext.getMessage("siscol.connect.error", null, UI.getCurrent().getLocale()),
						f);
				Notification.show(
						applicationContext.getMessage("siscol.connect.error", null, UI.getCurrent().getLocale()),
						Type.WARNING_MESSAGE);
			}
			Notification.show(applicationContext.getMessage("candidat.sync.apo.ok", null, UI.getCurrent().getLocale()),
	...
	private Candidat getCandidatByApogeeData(WSIndividu individuApogee, Candidat candidat) {
		...
		if (individuApogee.getIsWs()) {
			...
			/* Champs pays naissance */
			candidat.setSiScolPaysNaiss(
					getPaysNaissance(individuApogee.getCodTypDepPayNai(), individuApogee.getCodDepPayNai()));

			/* Champs dpt naissance */
			candidat.setSiScolDepartement(
					getDepartementNaissance(individuApogee.getCodTypDepPayNai(), individuApogee.getCodDepPayNai()));
		}
		...
		/* Champs civilite */
		candidat.setDatNaissCandidat(
				individuApogee.getDateNaiInd().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
	...
	public void deleteCandidat(CompteMinima cptMin, CandidatAdminListener listener) {
		if (cptMin.getCandidat() != null && cptMin.getCandidat().getCandidatures().size() > 0) {
			Notification.show(
					applicationContext.getMessage("candidat.delete.has.candidature", null, UI.getCurrent().getLocale()),
					Type.WARNING_MESSAGE);
			return;
		}
		ConfirmWindow win = new ConfirmWindow(
				applicationContext.getMessage("candidat.delete.window", null, UI.getCurrent().getLocale()));
		win.addBtnOuiListener(e -> {
			try {
				deleteCandidatBase(cptMin);
				userController.setNoDossierNomCandidat(null);
				Notification.show(
						applicationContext.getMessage("candidat.delete.ok", null, UI.getCurrent().getLocale()),
						Type.TRAY_NOTIFICATION);
				MainUI.getCurrent().buildMenuGestCand(false);
			} catch (Exception ex) {
				logger.error(applicationContext.getMessage("candidat.delete.error", null, UI.getCurrent().getLocale()),
						ex);
				Notification.show(
						applicationContext.getMessage("candidat.delete.error", null, UI.getCurrent().getLocale()),
						Type.WARNING_MESSAGE);
				return;
			}
		});
	...
	/**
	 * @param candidat
	 * @return true si le candidat est un candidat et qu'il a déjà au moins une
	 *         candidature
	 */
	public Boolean isCandidatAndHaveCandidature(Candidat candidat) {
