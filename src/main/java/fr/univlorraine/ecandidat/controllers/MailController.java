...
public class MailController {
	...
	// public String testMail(final Mail mail, final Map<String, Object> map) throws ParseException {
	// StringBuilder builder = new StringBuilder("<h2>Mail " + mail.getCodMail() + "</h2>");
	// /*Traitement du sujet*/
	// for (I18nTraduction trad : mail.getI18nSujetMail().getI18nTraductions()) {
	// builder.append("<h3>Sujet (" + trad.getLangue().getLibLangue() + ")</h3>");
	// builder.append(parseVelocity(transformI18nVelocity(trad.getValTrad()), map));
	// }
	//
	// /*Traitement du corps*/
	// for (I18nTraduction trad : mail.getI18nCorpsMail().getI18nTraductions()) {
	// builder.append("<h3>Corps (" + trad.getLangue().getLibLangue() + ")</h3>");
	// builder.append(parseVelocity(transformI18nVelocity(trad.getValTrad()), map));
	// }
	//
	...
	// /*Remplacement Velocity*/
	// private String replaceIfFromMail(String contentMail) {
	// /* Les balsies IF */
	// String baliseDebutIf = "{if($";
	// String baliseFinIf = ")}";
	//
	...
	// String property = contentMail.substring(indexDebutIf + baliseDebutIf.length(), indexFinIf);
	// String endIf = "{endif($" + property + ")}";
	// contentMail = contentMail.replace(baliseDebutIf + property + baliseFinIf, "#if(!$StringUtils.isEmpty($" + property + "))");
	// return contentMail.replace(endIf, "#end");
	// }
	// }
	// return null;
	// }

	// private String replacePropertyFromMail(final String contentMail) {
	// String baliseDebProp = "${";
	// String baliseFinProp = "}";
	// System.out.println("Debut prop");
	// /* On recherche la position de la balise debut */
	// int indexDebutIf = contentMail.indexOf(baliseDebProp, 0);
	// if (indexDebutIf != -1) {
	// System.out.println("trouvé deb : " + indexDebutIf);
	// /* elle existe donc on recherche la premiere position de la balise de fin du if */
	// int indexFinIf = contentMail.indexOf(baliseFinProp, indexDebutIf + baliseDebProp.length());
	// System.out.println("trouvé fin : " + indexFinIf);
	// if (indexFinIf != -1) {
	// System.out.println("substr : " + contentMail.substring(indexDebutIf + baliseDebProp.length(), indexFinIf));
	// /* Elle existe, on calcul la propriété */
	// String property = contentMail.substring(indexDebutIf + baliseDebProp.length(), indexFinIf);
	// return contentMail.replace(baliseDebProp + property + baliseFinProp, "$" + property);
	// }
	// }
	// return null;
	// }

	// public String transformI18nVelocity(String valeur) {
	// String contentWithoutProp = replacePropertyFromMail(valeur);
	// while (contentWithoutProp != null) {
	// valeur = contentWithoutProp;
	// contentWithoutProp = replacePropertyFromMail(valeur);
	// }
	// String contentWithoutIf = replaceIfFromMail(valeur);
	// while (contentWithoutIf != null) {
	// valeur = contentWithoutIf;
	// contentWithoutIf = replaceIfFromMail(valeur);
	// }
	//
	...
	// }
	//
	// private Map<String, Object> getVarContextVelocity(final String varMail, final MailBean mailBean, final String varCandidature, final CandidatureMailBean candidatureMailBean) {
	// Map<String, Object> map = new HashMap<>();
	// if (mailBean != null && varMail != null && !varMail.equals("")) {
	// String[] tabSplit = varMail.split(";");
	//
	// for (String property : tabSplit) {
	// map.put(property, mailBean.getValueProperty(property));
	// }
	// }
	// if (candidatureMailBean != null && varCandidature != null && !varCandidature.equals("")) {
	// String[] tabSplit = varCandidature.split(";");
	//
	// for (String property : tabSplit) {
	// map.put(property, candidatureMailBean.getValueProperty(property));
	// }
	// }
	// return map;
	// }
	//
	// private String parseVelocity(final String contentToParse, final Map<String, Object> map) throws ParseException {
	// StringReader reader = new StringReader(contentToParse);
	// Template template = new Template();
	// RuntimeServices runtimeServices = RuntimeSingleton.getRuntimeServices();
	// template.setRuntimeServices(runtimeServices);
	// template.setData(runtimeServices.parse(reader, "Template_ecandidat"));
	// template.initDocument();
	//
	// VelocityContext vc = new VelocityContext();
	// vc.put("StringUtils", StringUtils.class);
	// map.forEach((k, v) -> vc.put(k, v));
	//
	...
	//
	// /** Envoie un mail */
	// public void sendMailVelocity(final String mailAdr, final Mail mail, final MailBean bean, final Candidature candidature, String locale, final PdfAttachement attachement) {
	// if (mail == null || !mail.getTesMail()) {
	// return;
	// }
	// if (locale == null) {
	...
	//
	// /*Calcul du BCC*/
	// String bcc = (candidature != null && candidature.getFormation().getCommission().getCentreCandidature().getTemSendMailCtrCand())
	// ? candidature.getFormation().getCommission().getCentreCandidature().getMailContactCtrCand()
	// : null;
	//
	...
	// String varCandidature = getVarMailCandidature(mail.getCodMail());
	//
	// /*Association des property et des variables*/
	// Map<String, Object> map = getVarContextVelocity(varMail, bean, varCandidature, candidatureMailBean);
	//
	// try {
	// /*Traitement du sujet*/
	// String sujetMail = parseVelocity(i18nController.getI18nTraduction(mail.getI18nSujetMail(), locale), map);
	//
	// /*Traitement du content*/
	// String contentMail = parseVelocity(i18nController.getI18nTraduction(mail.getI18nCorpsMail(), locale), map);
	//
	// sendMail(mailAdr, sujetMail, contentMail, bcc, attachement);
	// } catch (ParseException e) {
	// logger.error("Impossible de parser le mail, une erreur est présente dans le template " + mail.getCodMail(), e);
	// return;
	// }
	// }
}
