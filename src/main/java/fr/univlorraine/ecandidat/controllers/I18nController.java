...
public class I18nController {
	...
	public I18n saveI18n(final I18n i18nMaybeIncomplet) {
		if (i18nMaybeIncomplet.getIdI18n() == null) {
			final I18n i18n = i18nRepository.save(new I18n(i18nMaybeIncomplet.getTypeTraduction()));
	...
	public I18n saveI18nNullable(final I18n i18n) {
		...
		final Iterator<I18nTraduction> iter = i18n.getI18nTraductions().iterator();
		while (iter.hasNext()) {
			final I18nTraduction trad = iter.next();
	...
	public TypeTraduction getTypeTraduction(final String typeTraduction) {
		return typeTraductionRepository.findById(typeTraduction).orElse(null);
	}
	...
	public String getI18nTraduction(final I18n i18n, final String codLangueCand) {
		if (i18n == null || i18n.getI18nTraductions().size() == 0) {
			...
			if (codLangueCand != null) {
				final Optional<I18nTraduction> i18nTraductionPref = i18n.getI18nTraductions().stream().filter(t -> t.getLangue().getTesLangue()
				...
				final Optional<I18nTraduction> i18nTraductionDefault = i18n.getI18nTraductions().stream().filter(t -> t.getLangue().equals(cacheController.getLangueDefault())).findFirst();
	...
	private String getCodeLangueActive(final String codLangue) {
		final String codLangueDefault = cacheController.getLangueDefault().getCodLangue();
		if (!codLangue.equals(codLangueDefault)) {
			final Optional<Langue> langueFilter = cacheController.getLangueEnServiceWithoutDefault().stream().filter(e -> e.getCodLangue().equals(codLangue)).findAny();
	...
	public String getLangueCandidat() {
		final SecurityUserCandidat user = userController.getSecurityUserCandidat();
		if (user != null) {
	...
	public void initLanguageUI(final Boolean forceToReloadMenu) {
		...
		if (langue != null) {
			...
			final Langue langueDefault = cacheController.getLangueDefault();
	...
	private Boolean changeLangueUI(final String codeLangue, final Boolean forceToReloadMenu) {
		...
		if (forceToReloadMenu || locale == null
			...
			UI.getCurrent().setLocale(new Locale(codeLangue));
